<template>
  ...
  <Transition name="intro-warp">
    <div v-if="showIntro" class="intro-overlay" @click="handleIntroClick">
      ...
      <div class="intro-center" :class="{ 'intro-ready': introPhase >= 1 }">
        ...
        <button class="intro-skip" :class="{ 'skip-show': introPhase >= 1 }" @click="skipIntro">
          {{ progressWidth >= 100 && autoSkipCancelled ? '[ ENTER SYSTEM ]' : '[ SKIP INTRO ]' }}
        </button>
      </div>
    </div>
  </Transition>
...
.intro-center.intro-ready { opacity: 1; transform: translateY(-50%); }
...
.intro-title {
  ...
  margin: 0 0 14px;
  color: #fff;
  text-shadow: 0 4px 12px rgba(0,0,0,0.9), 0 0 20px rgba(0,240,255,0.6);
  ...
  box-shadow: 0 16px 40px rgba(0,0,0,0.8), inset 0 0 20px rgba(0,240,255,0.1);
}
...
const autoSkipCancelled = ref(false)
...
function handleIntroClick(e) {
  if (e.target.closest('button') || e.target.closest('.cip') || e.target.closest('.cnav')) return
  autoSkipCancelled.value = true
...
async function initCosmicScene() {
  ...
  function updateCameraZ() {
    ...
    // Shift the 3D scene to the left to balance the text on the right
    if (container.clientWidth > 768) {
      const offsetX = container.clientWidth * 0.15
      camera.setViewOffset(container.clientWidth, container.clientHeight, offsetX, 0, container.clientWidth, container.clientHeight)
    } else {
